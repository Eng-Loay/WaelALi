...
async function replaceAssignmentQuestions(assignmentId, questions = [], connection = pool) {
  ...
  for (const q of questions) {
    ...
    await connection.query(
      `INSERT INTO assignment_questions (assignment_id, question_text, image_url, pdf_url, sort_order)
       VALUES (?, ?, ?, ?, ?)`,
      [assignmentId, q.question_text.trim(), q.image_url || null, q.pdf_url || null, order],
    );
    order += 1;
  }
  return order - 1;
